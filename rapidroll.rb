...
# ----------------------------------------------
class Block
	attr_accessor :image, :x, :y, :moving_velocity, :is_obstacle, :longer_block
...
def create_obstacle(blocks, moving_velocity, is_obstacle)
...
def create_longerblock(blocks, moving_velocity, longer_block)
	block = Block.new(moving_velocity)
	if longer_block
	  block.longer_block = true
	  block.image = Gosu::Image.new("images/longer_block.png")
	end
	blocks << block
  end
...
class Main < Gosu::Window
	def initialize
		...
		@time_elapsed = 0					
		@obstacle_freq = 3	# frequency of generating obstacles
		@longer_freq = 3				
	end
	...
	def generate_blocks
		...
		if rand(@obstacle_freq) < 1 && !@blocks.any? {|block| block.is_obstacle == true} && !@blocks.empty?
			create_obstacle(@blocks, @blocks_moving_velocity, true)			# Create an obstacle
		elsif rand(@longer_freq) < 1 && !@blocks.any? {|block| block.longer_block == true} && !@blocks.empty?
			create_longerblock(@blocks, @blocks_moving_velocity, true)			# Create an longer block
		else
			create_obstacle(@blocks, @blocks_moving_velocity, false)			# Create a normal block
		end
